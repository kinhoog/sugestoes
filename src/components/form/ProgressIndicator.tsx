...
export function ProgressIndicator({ currentStep, totalSteps, labels }: ProgressIndicatorProps) {
  ...
  return (
    <aside className="rounded-[1.25rem] border border-white/70 bg-white/80 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur lg:sticky lg:top-24">
          ...
          return (
            <li
              ...
              className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm transition-all duration-200 motion-reduce:transition-none ${
                active
                  ? 'bg-brand-50 text-brand-900 shadow-[0_10px_30px_rgba(21,120,194,0.12)]'
                  : complete
                    ? 'text-slate-700'
                    : 'text-slate-400'
              }`}
