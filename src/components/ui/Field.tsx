...
function FieldShell({ label, helper, children }: FieldShellProps) {
  return (
    <label className="group block">
      <span className="portal-field-label text-sm font-semibold text-slate-800 dark:text-slate-100">{label}</span>
      {helper ? (
        <span className="portal-field-helper mt-0.5 block text-xs leading-5 text-slate-500 dark:text-slate-400">
          {helper}
        </span>
      ) : null}
