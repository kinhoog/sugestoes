...
function navClassName(isActive: boolean): string {
  return `inline-flex min-h-9 items-center justify-center gap-2 rounded-xl px-2.5 py-1.5 text-[13px] font-semibold transition-all duration-200 motion-reduce:transition-none ${
    isActive
      ...
      <header className="sticky top-0 z-30 border-b border-white/70 bg-white/88 shadow-[0_16px_46px_rgba(15,23,42,0.06)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/88 dark:shadow-[0_18px_52px_rgba(0,0,0,0.34)]">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-2.5 px-4 py-2.5 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-center justify-between gap-3">
              <Link to={ROTAS.adminDashboard} className="flex min-w-0 items-center gap-3">
                <span className="shrink-0 rounded-xl bg-white p-1.5 shadow-[0_10px_24px_rgba(15,23,42,0.08)] ring-1 ring-slate-100 dark:bg-white/95 dark:ring-white/10">
              ...
              <div className="flex items-center gap-2 lg:hidden">
                ...
                <button
                  type="button"
                  onClick={() => void logout()}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white/90 text-slate-700 shadow-[0_10px_26px_rgba(15,23,42,0.08)] transition-all duration-200 hover:border-brand-200 hover:text-brand-800 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:border-brand-500 dark:hover:text-cyan-100"
                  aria-label="Sair"
                  title="Sair"
                >
                  <LogOut size={16} />
            ...
            <div className="hidden items-center justify-end gap-2 lg:flex">
              <Link
                to={ROTAS.formulario}
                className="inline-flex min-h-9 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/80 px-2.5 py-1.5 text-[13px] font-semibold text-slate-700 shadow-[0_10px_26px_rgba(15,23,42,0.06)] transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-200 hover:text-brand-800 hover:shadow-[0_14px_34px_rgba(15,23,42,0.1)] dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200 dark:hover:border-brand-500 dark:hover:text-cyan-100"
              >
                <ExternalLink size={15} />
                Portal público
              </Link>
              <span className="inline-flex min-h-9 max-w-[230px] items-center gap-2 truncate rounded-xl border border-brand-100 bg-white/78 px-2.5 py-1.5 text-[13px] font-semibold text-brand-900 shadow-[0_8px_22px_rgba(15,23,42,0.05)] dark:border-cyan-400/20 dark:bg-slate-900/76 dark:text-cyan-100 dark:shadow-none">
          ...
          <div className="flex items-center justify-between gap-2 lg:hidden">
            <Link
              to={ROTAS.formulario}
              className="inline-flex min-h-9 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/80 px-2.5 py-1.5 text-[13px] font-semibold text-slate-700 shadow-[0_10px_26px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200"
            >
              <ExternalLink size={15} />
              Portal público
            </Link>
            <span className="hidden min-h-9 max-w-[48%] items-center gap-2 truncate rounded-xl border border-brand-100 bg-white/78 px-2.5 py-1.5 text-[13px] font-semibold text-brand-900 sm:inline-flex dark:border-cyan-400/20 dark:bg-slate-900/76 dark:text-cyan-100">
              <ShieldCheck size={15} className="shrink-0" />
              <span className="truncate">{email}</span>
            </span>
          </div>
