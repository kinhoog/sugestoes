...
const navItems = [
  { to: ROTAS.adminDashboard, label: 'Dashboard', icon: <LayoutDashboard size={17} /> },
  { to: ROTAS.adminSolicitacoes, label: 'Solicitações', icon: <ClipboardList size={17} /> },
] as const;

function navClassName(isActive: boolean): string {
  return `inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-200 motion-reduce:transition-none ${
    isActive
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <Link to={ROTAS.adminDashboard} className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden rounded-full bg-brand-50 px-2 py-0.5 text-[11px] font-bold uppercase tracking-[0.12em] text-brand-700 ring-1 ring-brand-100 sm:inline-flex dark:bg-brand-900/40 dark:text-cyan-200 dark:ring-brand-500/20">
                      Admin
                    </span>
                  </div>
            ...
            <div className="hidden items-center justify-end gap-2 lg:flex">
              <Link
                to={ROTAS.formulario}
                className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/80 px-3 py-2 text-sm font-semibold text-slate-700 shadow-[0_10px_26px_rgba(15,23,42,0.06)] transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-200 hover:text-brand-800 hover:shadow-[0_14px_34px_rgba(15,23,42,0.1)] dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200 dark:hover:border-brand-500 dark:hover:text-cyan-100"
              >
                <ExternalLink size={15} />
                Portal público
              </Link>
              <span className="inline-flex min-h-10 max-w-[230px] items-center gap-2 truncate rounded-xl border border-brand-100 bg-brand-50/80 px-3 py-2 text-sm font-semibold text-brand-900 dark:border-brand-500/20 dark:bg-brand-900/32 dark:text-cyan-100">
          ...
          <div className="flex items-center justify-between gap-2 lg:hidden">
            <Link
              to={ROTAS.formulario}
              className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/80 px-3 py-2 text-sm font-semibold text-slate-700 shadow-[0_10px_26px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200"
            >
              <ExternalLink size={15} />
              Portal público
            </Link>
            <span className="hidden min-h-10 max-w-[48%] items-center gap-2 truncate rounded-xl border border-brand-100 bg-brand-50/80 px-3 py-2 text-sm font-semibold text-brand-900 sm:inline-flex dark:border-brand-500/20 dark:bg-brand-900/32 dark:text-cyan-100">
              <ShieldCheck size={15} className="shrink-0" />
              <span className="truncate">{email}</span>
            </span>
          </div>
